...
nodes = table.nodes;
%iteracoes
iter_normal = table.x0_25_iter_generate;
iter_limit = table.x0_25_iter_generate_limit;
iter_2k = table.x0_25_iter_generate_2k;
iter_2n = table.x0_25_iter_generate_2n;
iter_greedy = table.x0_25_iter_greedy;

plot(nodes,log(iter_normal),'x',"Color",[0.9290 0.6940 0.250])
...
plot(nodes,log(2.^(nodes-3).*nodes.^2),"--","Color",[0.9290 0.6940 0.250])
...
%%%%%%%%%%%%%%%% tempo
time_normal = table.x0_25_time_generate;
time_limit = table.x0_25_time_generate_limit;
time_2k = table.x0_25_time_generate_2k;
time_2n = table.x0_25_time_generate_2n;
time_greedy = table.x0_25_time_greedy;
figure(2)
plot(nodes,log(time_normal),'x',"Color",[0.9290 0.6940 0.250])
...
num_normal = table.x0_25_num_elementos_generate;
num_limit = table.x0_25_num_elementos_generate_limit;
num_2k = table.x0_25_num_elementos_generate_2k;
num_2n = table.x0_25_num_elementos_generate_2n;
num_greedy = table.x0_25_num_elementos_greedy;

num_exhaustive = table_p1.x0_25_num_elementos_exhaustive;
figure(3)
plot(nodes,num_normal,'x',"Color",[0.9290 0.6940 0.250])
...
confs_normal = table.x0_25_confs_tested_generate;
confs_limit = table.x0_25_confs_tested_generate_limit;
confs_2k = table.x0_25_confs_tested_generate_2k;
confs_2n = table.x0_25_confs_tested_generate_2n;
confs_greedy = table.x0_25_tries_greedy;
figure(4)
plot(nodes,log(confs_normal),'x',"Color",[0.9290 0.6940 0.250])
title("Número de Configurações testadas (0.25)")
xlabel("Número de vértices")
...
legend(["$p=0.15$","$\min{({n\choose k},1000)}$","$p=\frac{1}{2^k}$","$p=\frac{1}{2^n}$","\textit{Greedy}"],'Location','northwest',Interpreter='latex')
hold off
saveas(gcf,"figs/confs_25.png")

%%%%%%%%%%%%%%%% basic operations
operations_normal = iter_normal+table.x0_25_tries_generate;
operations_limit = iter_limit+table.x0_25_tries_generate_limit;
operations_2k = iter_2k+table.x0_25_tries_generate_2k;
operations_2n = iter_2n+table.x0_25_tries_generate_2n;
operations_greedy = iter_greedy;
figure(5)
plot(nodes,log(operations_normal),'x',"Color",[0.9290 0.6940 0.250])
title("Operações básicas (0.25)")
xlabel("Número de vértices")
